		...
		<>
			<nav className="Navbar-content">
				<div className="flex items-center justify-between px-4 lg:px-0">
					...
					>
						{open ? (
							<RiCloseFill className="Icons-navbar" />
						) : (
							<RiMenuFill className="Icons-navbar" />
						)}
					</div>
				</div>
				<div
					className={`Navbar-content-item ${open ? "top-16" : "top-[-820px] "}`}
				>
					<ul
						className={`Navbar-list ${
							open
								? "top-16 opacity-100 "
								: "top-[-820px] lg:opacity-100 opacity-0"
						}`}
					>
